...
function CharacterView() {
    ...
    return (
        <div className="App">
            ...
            <div className="character-profile-header">
                ...
                <div className="profile-image-container">
                    {editingField === 'image' ? (
                        <div className="inline-edit-group" style={{ flexDirection: 'column' }}>
                            ...
                            {editData.image_url && <img src={editData.image_url} alt="Preview" className="profile-image-preview" />}
                        ...
                        <div className="profile-image-display" onClick={() => setEditingField('image')} title="Click to edit portrait">
                            {character.image_url ? (
                ...
                <div className="profile-identity">
                    ...
                    {editingField === 'alias' ? (
                        ...
                        <p className="editable-field story-date" style={{ textTransform: 'none', letterSpacing: 'normal', fontSize: '1.2rem', fontStyle: 'italic', margin: 0 }}>
            ...
            <h2 style={{ borderBottom: '1px solid var(--border-light)', paddingBottom: '0.5rem', marginTop: '3rem' }}>Featured In</h2>
            {stories.length === 0 ? (
                <p style={{ fontStyle: 'italic', color: 'var(--color-ink-muted)', textAlign: 'center' }}>This character has not been recorded in any tales yet.</p>
            ) : (
...
function ProfileSection({ title, value, editValue, isEditing, onEdit, onChange, onSave, onCancel }) {
    return (
        <div className="profile-section" style={{ marginBottom: '2rem', paddingBottom: '1rem', borderBottom: '1px solid var(--border-light)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '0.5rem' }}>
                <h3 style={{ margin: 0, fontFamily: 'var(--font-display)', color: 'var(--color-ink-soft)' }}>{title}</h3>
                {!isEditing && <button className="inline-edit-icon" onClick={onEdit} style={{ opacity: 0.5 }}>&#x270E;</button>}
            </div>
            ...
            {isEditing ? (
                ...
                <p style={{ margin: 0, color: 'var(--color-ink-soft)', whiteSpace: 'pre-wrap' }}>
                    {value || <span style={{ fontStyle: 'italic', color: 'var(--color-ink-muted)' }}>No {title.toLowerCase()} provided.</span>}
                </p>
            )}
        </div>
    );
